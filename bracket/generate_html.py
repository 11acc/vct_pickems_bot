...
def generate_bracket_html(bracket_format: str, bracket_data: dict) -> str | None:
    if bracket_format == "8d22":
        return generate_bracket_html_8d22(bracket_data)
    elif bracket_format == "8d00":
        return generate_bracket_html_8d00(bracket_data)


# Bracket generator for format 8d22
def generate_bracket_html_8d22(bracket_data: dict) -> str | None:
    if not bracket_data:
    ...
    css_path = os.path.join(os.path.dirname(__file__), "./styling/style.css")
...
# Helper function to get line modifier
def get_line_mod(match_idx: int, round_name: str, bracket_type: str) -> str:
    if bracket_type == "upper":
        if round_name == "Upper Quarterfinals":
            return "mod-down" if match_idx in [0, 2] else "mod-up"
        elif round_name == "Upper Semifinals":
...
# Bracket generator for format 8d00
def generate_bracket_html_8d00(bracket_data: dict) -> str | None:
    if not bracket_data:
        print(f"Null input bracket data: {bracket_data}")
        return None
    bracket_format = "8d00"

    # Read css from master style and additional format specific styling
    css_content = ""
    ...
    extra_css_path = os.path.join(os.path.dirname(__file__), f"./styling/style_{bracket_format}.css")
    if not os.path.exists(extra_css_path):
        raise FileNotFoundError(f"Template file not found at: {extra_css_path}")

    with open(css_path, "r") as css_file:
        css_content = css_file.read()
    with open(extra_css_path, "r") as extra_css_file:
        css_content += extra_css_file.read()
    ...
    match_navigation = {
        "upper": {
            0: {
                0: {"curr_id": "111", "next_id": "211", "next_pos": "0", "next_id2": "112", "next_pos2": "0"},
                1: {"curr_id": "121", "next_id": "221", "next_pos": "1", "next_id2": "122", "next_pos2": "1"},
                2: {"curr_id": "131", "next_id": "221", "next_pos": "0", "next_id2": "122", "next_pos2": "0"},
                3: {"curr_id": "141", "next_id": "221", "next_pos": "1", "next_id2": "122", "next_pos2": "1"}
            },
            1: {
                0: {"curr_id": "211", "next_id": "311", "next_pos": "0", "next_id2": "222", "next_pos2": "0"},
                1: {"curr_id": "221", "next_id": "311", "next_pos": "1", "next_id2": "212", "next_pos2": "0"}
    ...
    for col_idx, round_data in enumerate(bracket_data["upper"]):
        ...
        for match_idx, match in enumerate(round_data["matches"]):
            ...
            # Correct line directions
            mod_line = get_line_mod(match_idx, round_data["round"], "upper")
    ...
    for col_idx, round_data in enumerate(bracket_data["lower"]):
        ...
        for match_idx, match in enumerate(round_data["matches"]):
            ...
            # Correct line directions
            mod_line = get_line_mod(match_idx, round_data["round"], "lower")
